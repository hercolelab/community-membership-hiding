...
class Estimator(object):
    ...
    def _check_indexing(graph: ig.Graph):
        ...
        node_indices = sorted([node.index for node in graph.vs])

        assert numeric_indices == node_indices, "The node indexing is wrong."

    def _check_graph(self, graph: ig.Graph) -> ig.Graph:
...
class SCD(Estimator):
    ...
    def _set_omega(self):
        ...
        """
        self._omega = self._graph.transitivity_undirected()

    def _set_nodes(self):
        """
        Creating a list of nodes.
        """
        self._nodes = list(range(self._graph.vcount()))
    ...
    def _create_initial_partition(self):
        ...
        for v in self._graph.vs:
            cc = self._graph.transitivity_local_undirected([v.index], mode='zero')[0]
            self._clustering_coefficient[v.index] = cc
