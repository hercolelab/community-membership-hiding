...
class nablaCMH_proj():
    ...
    def community_membership_hiding(self, verbose_iterations: bool=False) -> Tuple[ig.Graph, int, dict, Optional[dict]]:
        ...
        goal: int = 0
        count_reinit: int = 0
        history_adj_vector: List[Tensor] = [self.a_u]
        history_gradient: List[Tensor] = []
        edges_changed: dict = {}
        ...
        while goal==0 and t < self.T:
            ...
            history_adj_vector.append(a_new)
            edges_changed, n_changes = nablaUtils.get_changes(history_adj_vector[-2], history_adj_vector[-1], self.u)
            ...
            loss.backward()
            optimizer.step()
            history_gradient.append(x_hat.grad)
            t += 1
            ...
            if budget_used > self.budget:
                if self.reinitialization: 
                    ...
                    goal = 0 
                    budget_used=0
                    history_adj_vector.append(self.a_u)
                    if t+1 < self.T:
                        history_gradient = []
        ...
        if goal==0 and budget_used != self.budget:
            budget_available = self.budget - budget_used
            beta = 0.9
            avg_grad = torch.zeros_like(x_hat.data)
            x_hat_projected = x_hat.detach().clone()
            a_u_prime = history_adj_vector[-1]
            for grad in history_gradient:
                avg_grad = beta * avg_grad + (1 - beta) * grad
            while budget_available > 0:
                x_hat_projected = x_hat_projected - self.lr * avg_grad
                p_hat_projected = torch.tanh(x_hat_projected)
                p_projected = nablaUtils.threshold_tanh(p_hat_projected,tp,tn)
                a_new_projected = nablaUtils.clamp(a_u_prime + p_projected)
                changes, n_changes = nablaUtils.get_changes(a_u_prime, a_new_projected, self.u)

                if n_changes > 0 and n_changes <= budget_available:
                    budget_available -= n_changes
                    a_u_prime = a_new_projected
                ...
                elif n_changes > 0 and n_changes > budget_available:
                    # The budget is not enough for all changes, select top-k based on score.
                    potential_changes = []
                    # scores for removed edges
                    for edge in changes["removed"]:
                        node_i = edge[1]
                        score = torch.abs(p_hat_projected[node_i]) * torch.abs(avg_grad[node_i])
                        potential_changes.append({'score': score.item(), 'type': 'remove', 'edge': edge})
                    # scores for added edges
                    for edge in changes["added"]:
                        node_i = edge[1]
                        score = torch.abs(p_hat_projected[node_i]) * torch.abs(avg_grad[node_i])
                        potential_changes.append({'score': score.item(), 'type': 'add', 'edge': edge})

                    # Sort changes by score in descending order
                    sorted_changes = sorted(potential_changes, key=lambda x: x['score'], reverse=True)

                    # Apply top-k changes until budget is exhausted
                    applied_count = 0
                    for change in sorted_changes:
                        if applied_count >= budget_available:
                            break
                        
                        edge_to_change = change['edge']
                        node_i = edge_to_change[1]
                        
                        if change['type'] == 'remove':
                            if g_prime.are_connected(*edge_to_change) or g_prime.are_connected(*edge_to_change[::-1]):
                                g_prime.delete_edges([edge_to_change])
                                a_u_prime[node_i] = 0
                                applied_count += 1
                        
                        elif change['type'] == 'add':
                            if not g_prime.are_connected(*edge_to_change) and not g_prime.are_connected(*edge_to_change[::-1]):
                                g_prime.add_edges([edge_to_change])
                                a_u_prime[node_i] = 1
                                applied_count += 1
                    
                    budget_available -= applied_count
                    budget_used += applied_count

            changes, _ = nablaUtils.get_changes(history_adj_vector[0], a_u_prime, self.u)
            del x_hat, p_hat, p, a_new, history_adj_vector, history_gradient
            return g_prime, budget_used, changes, nablaCMH_additional_results
        
        # ---- Projection ---- #
        elif goal==1 and budget_used != self.budget:
            budget_available = self.budget - budget_used
            beta = 0.9
            avg_grad = torch.zeros_like(x_hat.data)
            x_hat_projected = x_hat.detach().clone()
            ...
            changes, _ = nablaUtils.get_changes(history_adj_vector[0], history_adj_vector[-1], self.u)
            del x_hat, p_hat, p, a_new, history_adj_vector, history_gradient
            return g_prime, budget_used, changes, nablaCMH_additional_results
        else: 
            changes, _ = nablaUtils.get_changes(history_adj_vector[0], history_adj_vector[-1], self.u)
