...
@hydra.main(config_path=f"{ROOT_DIR}/src/conf", config_name="hyp_search", version_base=None)
def main(cfg: DictConfig) -> None:
    ...
    with open(f"{ROOT_DIR}/src/conf/hyp_search.yaml", "r") as file:
    ...
    with open(f"{ROOT_DIR}/src/conf/hyp_search.yaml", "w") as file:
    ...
    env = GraphEnvironment(
        ...
        graph_path=f"{ROOT_DIR}/{graph_path}",
    )
    ...
    wandb.agent(sweep_id, function=lambda: exp(env,save_path), count=1)
